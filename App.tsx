...
const App = () => {
  ...
  const handleFinalisePlan = async (clientId: string) => {
    ...
    try {
      ...
      alert("AI Protocol synthesis failed. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderContent = () => {
    ...
    if (currentClient.planStatus === 'CONSULTATION_SUBMITTED') {
      return (
        <div className="max-w-2xl mx-auto text-center py-24 px-6 bg-slate-900/40 rounded-[3rem] border border-slate-800 backdrop-blur-sm shadow-2xl animate-in zoom-in duration-500">
          <div className="text-6xl mb-8 animate-pulse">🧬</div>
          <h2 className="text-4xl font-black text-white mb-4 italic uppercase tracking-tighter">Analysing Phenotype</h2>
          <p className="text-slate-400 mb-10 max-w-md mx-auto">Hi {currentClient.profile.name}. Your MuskyFit coach is currently engineering your bespoke V-Taper protocol. We will notify you once your 12-week block is live.</p>
          <div className="flex justify-center gap-4">
            <button onClick={() => setRole('COACH')} className="bg-cyan-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-lg shadow-cyan-500/20 hover:bg-cyan-500 transition-all">Coach Command Center</button>
          </div>
        </div>
      );
    }
    ...
    switch (activeTab) {
        ...
        return currentClient.plan ? (
          <WorkoutTracker 
            ...
            onFinish={() => { 
              ...
              updateClientData(currentClient.id, { 
                currentWorkoutIndex: (currentClient.currentWorkoutIndex + 1) % currentClient.plan!.workoutSplit.length 
              }); 
            }} 
          />
        ) : null;
      ...
      case 'transformation': 
        return (
          <TransformationHub 
            photos={currentClient.photos} 
            onUpload={(p) => updateClientData(currentClient.id, { photos: [p, ...currentClient.photos] })} 
            biometrics={`${currentClient.intake?.weight}kg, ${currentClient.intake?.height}cm`} 
          />
        );
      case 'reviews': 
          ...
          <div className="space-y-6 pb-24 animate-in fade-in duration-700">
            ...
            <div className="bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-800 flex justify-between items-center shadow-xl">
              <div>
                ...
                <p className="text-[10px] text-cyan-500 uppercase tracking-[0.4em] font-black mt-1 italic">Elite Protocol Active</p>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div 
                className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 flex flex-col justify-between group hover:border-cyan-500 transition-all cursor-pointer shadow-2xl relative overflow-hidden" 
                onClick={() => setActiveTab('workout')}
              >
                 <div className="absolute top-0 right-0 p-8 opacity-5 text-6xl font-black italic uppercase select-none">Lift</div>
                 <div className="relative z-10">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest italic">Next Task</h3>
                 ...
                 <button className="w-full py-5 bg-white text-black font-black uppercase tracking-[0.2em] rounded-2xl text-[10px] group-hover:bg-cyan-500 group-hover:text-white transition shadow-xl italic">Execute Protocol</button>
              </div>
